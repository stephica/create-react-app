...
const GraphQl = ({ search, sender, data: { loading, getBySender, refetch } }) => {
  ...
  return (
    <Buffer>
      ...
      <Input fluid style={{ maxWidth: '600px' }} action={<Button onClick={clickhandler}> Search </Button>} />
      <div style={{ marginTop: '20px' }}>
        {getBySender && !loading && getBySender.map((tx, i) => <Tx {...tx} key={i} />)}
        {loading && 'loading...'}
        {getBySender && !getBySender.length && !loading && 'No results, try searching for: 0xc7d3e431be6222543364408a94c12c0d089be305'}
      </div>
    </Buffer>
  )
}
