...
export const getTotalBalance = addresses => {
  if (addresses) {
    const total = addresses.map(el => Number(el.balance)).reduce((a, b) => Number(a) + Number(b), 0).toFixed(5)
...
export const getGroups = (addresses = [], wallets = []) => {
  ...
  return IdGroups.map(id => {
    return {
      wallet: id ? wallets.filter(wallet => isMatchingOrBothNull(wallet._id, id))[0] : placeholderWalletForNull,
      addresses: addresses.filter(info => isMatchingOrBothNull(info.wallet, id))
    }
  })
}
